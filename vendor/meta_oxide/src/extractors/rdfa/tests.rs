...
#[test]
fn test_rdfa_property_typeof_same_element_no_recursion() {
    // Regression: an element bearing BOTH `property` and `typeof` used
    // to send item/value extraction into unbounded mutual recursion
    // (process-killing stack overflow). The typeof side is captured by
    // the item itself; the property falls back to the text value.
    let html = r#"<div typeof="summary" property="x">text</div>"#;
    let result = extract(html, None).unwrap();
    assert_eq!(result.len(), 1);
    assert_eq!(result[0].type_of, Some(vec!["summary".to_string()]));
    assert_eq!(
        result[0].properties.get("x"),
        Some(&vec![RdfaValue::Literal("text".to_string())])
    );
}

#[test]
fn test_rdfa_property_typeof_nested_no_recursion() {
    // Same cycle through a nested child: the nested typeof becomes the
    // value of its own property (pre-existing merge semantics for
    // typeof children whose parent carries no property) instead of
    // recursing.
    let html = r#"<div typeof="outer"><div typeof="inner" property="p">text</div></div>"#;
    let result = extract(html, None).unwrap();
    assert_eq!(result.len(), 1);
    assert_eq!(result[0].type_of, Some(vec!["outer".to_string()]));
    let values = result[0].properties.get("p").expect("p collected");
    assert_eq!(values.len(), 1);
    match &values[0] {
        RdfaValue::Item(nested) => {
            assert_eq!(
                nested.type_of,
                Some(vec!["inner".to_string()])
            );
            assert_eq!(
                nested.properties.get("p"),
                Some(&vec![RdfaValue::Literal("text".to_string())])
            );
        }
