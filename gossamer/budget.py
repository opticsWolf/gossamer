...
class ContentBudget:
    ...
    def _truncate(
        ...
        """
        # Implemented in Rust (src/budget.rs).
        return _rust.budget_truncate(
            text, char_limit, token_limit, self._tb.model_name
        )
    ...
    def _content_budget(self) -> tuple[int, int]:
        ...
        """
        chars, tokens = _rust.budget_content_split(
            self._tb.max_markdown_chars, self._tb.max_tokens,
            self._tb.link_budget_ratio,
        )
        return chars, tokens
    @staticmethod
    def _shrink_parsed_payload(payload_json: str, budget: Optional[int]) -> str:
        ...
        if budget is None:
            return payload_json
        # Implemented in Rust (src/budget.rs).
        return _rust.shrink_payload_json(payload_json, budget)
    ...
    def _shrink_research(result: dict, budget: Optional[int]) -> str:
        ...
        # Implemented in Rust (src/budget.rs) over a JSON snapshot.
        return _rust.shrink_research_json(json.dumps(out), budget)

    def _json_fits(self, text: str, char_limit: int, token_limit: int) -> bool:
        """True when *text* is inside both budgets."""
        # Implemented in Rust (src/budget.rs).
        return _rust.budget_json_fits(
            text, char_limit, token_limit, self._tb.model_name
        )
