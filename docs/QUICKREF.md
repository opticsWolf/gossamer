...
| `download_file` | `gossamer download URL -o PATH` | `--min-bytes 1`, `--max-bytes 0` (configured cap), `--expect-format auto\|pdf`, `--overwrite`, `--try-mirrors URL…` (caller-supplied, sequential, policy-checked); streams to an atomic file, does not extract |
