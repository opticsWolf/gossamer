...
| Crawl discovery seeds (v0.4.8) | ✅ | `search_prior` (opt-in, E1): one site-scoped search (`site:<host> <focus>`, top 5) feeds results into the frontier at depth 1 with a rank bonus +0.1/(rank+1), exempt from `min_score` (engine already ranked them), document results routed to `documents`, any search failure fail-open (warning logged, link-graph crawl proceeds, no crawl `errors`); payload echoes `search_prior` and, when on, `search_results` (eligible count); gated to `max_depth >= 1` since results are depth 1. `seed_urls` (E2): caller/agent-supplied start URLs normalised against the root, SSRF-checked in full (S1), pushed at depth 0 (children depth 1), subject to `min_score` (`"seed below min score"` / `"ssrf blocked"` skip reasons, never silent); a seed equal to the root is a no-op; fetch failures are non-fatal `errors` that do not consume budget. E3: cross-modal loop (crawl → documents → extract_document → its links → next crawl with `seed_urls`) is the documented agent pattern, integration-tested |
