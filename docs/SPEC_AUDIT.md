...
| Focused crawl (v0.4.6) | ✅ | `focused_discovery(root_url, query=None, max_depth=3, max_pages=15, same_host=False, min_score=0.05)`: BFS over the link graph with a relevance-ranked frontier (`0.7 × query coverage + 0.3 × containing-page topic coverage`, effective `score × 0.7^depth`; ties by discovery order so flat scores degrade to plain BFS); page budget counts successful fetches only; boilerplate paths and static assets skipped; document links collected, never fetched; full pages stay in the page cache for in-full `inspect_html_page` re-reads; global budget enforced |
| Semantic crawl scoring (v0.4.8) | ✅ | Frontier score upgraded to BM25/IDF: term weights from a running document-frequency corpus fed after every fetch (flat until 3 pages read, so the scorer starts exactly like v0.4.6); anchor context (±50 chars around the anchor in the rendered body, ≤8 tokens, cached per page, fail-open) joins the label; URL path priors (×1.15 docs-ish, ×0.85 transactional) gated to the ≥3-page regime; offline thesaurus (`thesaurus.json`, 31 clusters / ~230 curated terms, no generics) expands the query at half weight, capped at 2× base, deterministic, loader fail-open; query echo reports additions (`"deep learning +2"`); legacy no-corpus `_crawl_score` is bit-for-bit v0.4.6 |
| Crawl richness + ranked documents (v0.4.8) | ✅ | per-page `content_chars` (full delivered size, pre-skim) and `term_hits` (query-term occurrences over the full body token stream); opt-in `excerpts` (keyword-densest 300-char window, step 100, densest wins, ties earliest, zero density omitted, ellipses mark a window not touching the head/tail); `documents` is a rank-ordered `{url, anchor, score}` list scored at first sighting with the live corpus (depth 0, no decay) and floored by `min_score` (`documents_below_score` counter + `skipped` reason `below min score`), never fetched; M11 shrink path regression-tested with stats + excerpts on |
| Crawl discovery seeds (v0.4.8) | ✅ | `search_prior` (opt-in, E1): one site-scoped search (`site:<host> <focus>`, top 5) feeds results into the frontier at depth 1 with a rank bonus +0.1/(rank+1), exempt from `min_score` (engine already ranked them), document results routed to `documents`, any search failure fail-open (warning logged, link-graph crawl proceeds, no crawl `errors`); payload echoes `search_prior` and, when on, `search_results` (eligible count); gated to `max_depth >= 1` since results are depth 1. `seed_urls` (E2): caller/agent-supplied start URLs normalised against the root, SSRF-checked in full (S1), pushed at depth 0 (children depth 1), subject to `min_score` (`"seed below min score"` / `"ssrf blocked"` skip reasons, never silent); a seed equal to the root is a no-op; fetch failures are non-fatal `errors` that do not consume budget. E3: cross-modal loop (focused_discovery → documents → extract_document → its links → next focused_discovery with `seed_urls`) is the documented agent pattern, integration-tested |
